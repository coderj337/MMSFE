...
from COMG_model.modules.base_cmn import BaseCMN
from COMG_model.modules.visual_extractor import VisualExtractor
import copy
cp = copy.deepcopy
...
class BaseCMNModel(nn.Module):
    ...
    def forward_iu_xray(self, 
        ...
        # fc_feats(形状: [batch_size, 2048])：每个样本的平均特征。
        fc_feats = torch.cat((fc_feats_0, fc_feats_1), dim=1)
        # att_feats(形状: [batch_size, 49, 512])：注意力特征。
        att_feats = torch.cat((att_feats_0, att_feats_1), dim=1)
        # mask_feature(形状: [batch_size, 196, 512])：每个样本的掩码特征。
        mask_feats = torch.cat((mask_feature_0, mask_feature_1), dim=1)
        # disease_token_feature (形状: [batch_size, 4, 512])：每个样本的疾病标记特征。
