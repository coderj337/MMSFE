...
def sort_pack_padded_sequence(input, lengths):
    # 1. 对序列长度进行排序，得到排序后的长度和对应的索引
    sorted_lengths, indices = torch.sort(lengths, descending=True)

    # 2. 根据排序后的索引对输入序列进行重排序
    tmp = pack_padded_sequence(input[indices], sorted_lengths.cpu(), batch_first=True)

    # 3. 创建一个与 indices 相同的张量，并将其初始化为索引本身
    inv_ix = indices.clone()

    # 4. 使用原索引创建一个逆序索引，以便恢复原序列顺序
...
def pack_wrapper(module, att_feats, att_masks):
    if att_masks is not None:
        ...
        # 2. 通过打包的序列运行模块
        processed = module(packed[0])

        # 3. 将结果包装成 PackedSequence 对象
        packed_output = PackedSequence(processed, packed[1])

        # 4. 对打包的序列进行填充并恢复原来的顺序
        return pad_unsort_packed_sequence(packed_output, inv_ix)
    else:
        # 如果没有掩码，直接通过模块处理
...
class AttModel(CaptionModel):
    ...
    # 这个方法用于根据注意力掩码剪裁注意力特征和掩码，使得它们的长度与最大序列长度一致
    ...
    # 这个方法用于根据给定特征生成字幕序列。它支持多种采样方法，如贪婪搜索、束搜索等
