...
class R2DataLoader(DataLoader):
    def __init__(self, args, tokenizer, split, shuffle):
        ...
        if self.dataset_name == 'iu_xray':
            # dataset：(image_id, image, image_mask_bone, image_mask_lung, image_mask_heart, image_mask_mediastinum,
            #       report_ids, report_masks, seq_length, disease_detected, disease_detected_len)
    ...
    # 将输入的数据进行批处理（batching）和填充（padding）操作，以便于后续的模型训练或推理
    @staticmethod
