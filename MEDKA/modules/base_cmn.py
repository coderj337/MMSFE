...
def memory_querying_responding(query, key, value, mask=None, dropout=None, topk=32):
    ...
    # 对选择的分数应用softmax得到注意力概率
    p_attn = F.softmax(selected_scores, dim=-1)

    # 如果提供了dropout，应用dropout
    if dropout is not None:
        p_attn = dropout(p_attn)
...
# 基于 Transformer 的序列到序列模型。该模型包括一个编码器、一个解码器、源嵌入和目标嵌入层，以及一个通用的注意力机制模块（cmn）
...
# 用于实现 Transformer 模型中的子层连接，包括残差连接、层归一化和 dropout
