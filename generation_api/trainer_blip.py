...
class BaseTrainer(object):
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            result = self._train_epoch_dcl(epoch)
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch_dcl(self, epoch):
        ...
        with torch.no_grad():
            ...
            d_val = dict(zip(b, a))
            item_val = []
            for batch_idx, (images, captions, knowledge_skg, image_path) in enumerate(self.val_dataloader):
                images = images.to(self.device)
                ground_truths = captions
                reports, knowledge_used = self.model.generate(images, knowledge_skg, sample=False, num_beams=3, max_length=90, min_length=5)
        ...
        with torch.no_grad():
            ...
            d_test = dict(zip(b, a))
            item_test = []
            for batch_idx, (images, captions, knowledge_skg, image_path) in enumerate(self.test_dataloader):
                images = images.to(self.device)
                reports, knowledge_used = self.model.generate(images, knowledge_skg, sample=False, num_beams=3, max_length=90, min_length=5)
                ground_truths = captions
